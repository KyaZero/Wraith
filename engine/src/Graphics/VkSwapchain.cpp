...
vk::Format Wraith::SwapChain::FindDepthFormat() const
{
...
vk::Result Wraith::SwapChain::SubmitCommandBuffers(const vk::CommandBuffer& buffers, u32& image_index)
{
    ...
    const vk::PresentInfoKHR present_info(*m_RenderFinishedSemaphores[m_CurrentFrame], *m_SwapChain, image_index);
    // we need to use regular vk functions here, as Vulkan.hpp will assert for certain VkResults which aren't actually erronous.
...
{
    auto [capabilities, formats, present_modes] = m_Device.GetSwapChainSupport();
    ...
    const auto indices = m_Device.FindPhysicalQueueFamilies();
    const u32 queue_family_indices[] = { indices.graphics_family, indices.present_family };
...
void Wraith::SwapChain::CreateRenderPass()
{
    ...
    const vk::RenderPassCreateInfo render_pass_info({}, attachments, subpass, dependency);
...
vk::Extent2D Wraith::SwapChain::ChooseExtent(const vk::SurfaceCapabilitiesKHR& capabilities) const
{
