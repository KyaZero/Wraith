...
char const *tinyfd_getGlobalChar(
    char const *aCharVariableName) /* to be called from C# (you don't need this in C or C++) */
{
...
static void hiddenConsoleW(wchar_t const *aString, wchar_t const *aDialogTitle, int aInFront)
{
    ...
    if (!CreateProcessW(
            NULL, (LPWSTR)aString, NULL, NULL, FALSE, CREATE_NEW_CONSOLE, NULL, NULL, &StartupInfo, &ProcessInfo))
    {
        return; /* GetLastError(); */
    }
                                ...
                                wchar_t const *aSingleFilterDescription) /* NULL or "image files" */
{
    ...
    if (quoteDetectedW(aTitle))
        return tinyfd_saveFileDialogW(L"INVALID TITLE WITH QUOTES",
                                      aDefaultPathAndFile,
                                      aNumOfFilterPatterns,
                                      aFilterPatterns,
                                      aSingleFilterDescription);
    if (quoteDetectedW(aDefaultPathAndFile))
        return tinyfd_saveFileDialogW(aTitle,
                                      L"INVALID DEFAULT_PATH WITH QUOTES",
                                      aNumOfFilterPatterns,
                                      aFilterPatterns,
                                      aSingleFilterDescription);
    if (quoteDetectedW(aSingleFilterDescription))
        return tinyfd_saveFileDialogW(aTitle,
                                      ...
                                      aNumOfFilterPatterns,
                                      aFilterPatterns,
                                      L"INVALID FILTER_DESCRIPTION WITH QUOTES");
                                ...
                                int aAllowMultipleSelects) /* 0 or 1 ; -1 to free allocated memory and return */
{
    ...
    if (quoteDetectedW(aTitle))
        return tinyfd_openFileDialogW(L"INVALID TITLE WITH QUOTES",
                                      aDefaultPathAndFile,
                                      aNumOfFilterPatterns,
                                      aFilterPatterns,
                                      aSingleFilterDescription,
    ...
    if (quoteDetectedW(aDefaultPathAndFile))
        return tinyfd_openFileDialogW(aTitle,
                                      L"INVALID DEFAULT_PATH WITH QUOTES",
                                      aNumOfFilterPatterns,
                                      aFilterPatterns,
                                      aSingleFilterDescription,
    ...
    {
        if (quoteDetectedW(aFilterPatterns[i]))
            return tinyfd_openFileDialogW(
                L"INVALID FILTER_PATTERN WITH QUOTES", aDefaultPathAndFile, 0, NULL, NULL, aAllowMultipleSelects);
    }
...
static int inputBoxWinGui(char *aoBuff,
...
static char *saveFileDialogWinGui(char *aoBuff,
                                  char const *aTitle,                   /* NULL or "" */
                                  ...
                                  char const *aSingleFilterDescription) /* NULL or "image files" */
{
    ...
    lTmpWChar = tinyfd_saveFileDialogW(lTitle,
                                       lDefaultPathAndFile,
                                       aNumOfFilterPatterns,
                                  ...
                                  int aAllowMultipleSelects)            /* 0 or 1 */
{
    ...
    lTmpWChar = tinyfd_openFileDialogW(lTitle,
                                       lDefaultPathAndFile,
                                       aNumOfFilterPatterns,
                                       (wchar_t const **)lFilterPatterns, /*stupid cast for gcc*/
                                       lSingleFilterDescription,
...
static int inputBoxWinConsole(char *aoBuff,
                              char const *aTitle,        /* NULL or "" */
                              char const *aMessage,      /* NULL or "" may NOT contain \n nor \t */
                              char const *aDefaultInput) /* "" , if NULL it's a passwordBox */
{
...
static char *saveFileDialogWinConsole(char *aoBuff,
                                      char const *aTitle,              /* NULL or "" */
                                      char const *aDefaultPathAndFile) /* NULL or "" */
{
    char lDialogString[MAX_PATH_OR_CMD];
...
static char *selectFolderDialogWinConsole(char *aoBuff,
                                          char const *aTitle,       /*  NULL or "" */
                                          char const *aDefaultPath) /* NULL or "" */
{
    char lDialogString[MAX_PATH_OR_CMD];
...
static int perlPresent(void)
{
    ...
    if (lPerlPresent < 0)
    {
        ...
        if (lPerlPresent)
        {
            lIn = popen("perl -MNet::DBus -e \"Net::DBus->session->get_service('org.freedesktop.Notifications')\" 2>&1",
                        "r");
            if (fgets(lBuff, sizeof(lBuff), lIn) == NULL)
            {
                lPerlPresent = 2;
...
void tinyfd_beep(void)
{
    ...
    else if (speakertestPresent())
    {
        ...
        strcpy(lDialogString,
               "( speaker-test -t sine -f 440 > /dev/tty )& pid=$!;sleep .4; kill -9 $pid"); /*.3 was too short for mac
                                                                                                g3*/
    }
 ...
 -e 'else' -e 'return 0' -e 'end if' ");
    ...
    else if (tfd_zenityPresent() || tfd_matedialogPresent() || tfd_shellementaryPresent() || tfd_qarmaPresent())
    {
        if (tfd_zenityPresent())
        {
            ...
            {
                strcat(lDialogString,
                       " --attach=$(sleep .01;xprop -root 32x '\t$0' _NET_ACTIVE_WINDOW | cut -f 2)"); /* contribution:
                                                                                                          Paul Rouget */
            }
        }
        ...
        else
        {
            ...
            {
                strcat(lDialogString,
                       " --attach=$(xprop -root 32x '\t$0' _NET_ACTIVE_WINDOW | cut -f 2)"); /* contribution: Paul
                                                                                                Rouget */
            }
        }
        strcat(lDialogString, " --");
