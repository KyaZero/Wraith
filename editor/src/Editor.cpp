...
namespace fw
{
	Editor::Editor() : Application("Editor"), m_CameraController(m_Engine->GetWindow()->GetSize().x, m_Engine->GetWindow()->GetSize().y, true), m_ActiveScene(nullptr)
	{
		...
		for (i32 y = -half_num; y < half_num; ++y)
		{
			for (i32 x = -half_num; x < half_num; ++x)
			{
				...
				sprite.color = Vec4f{ x / (f32)half_num, y / (f32)half_num, (x / (f32)half_num + y / (f32)half_num) / 2.0f, 1 } + Vec4f{ 1.1f, 1.1f, 1.1f, 0.0f };
				sprite.layer = y;
	...
	void Editor::OnUIRender()
	{
		...
		//Draw first
		ImGui::Image(texture->GetShaderResourceView(), ImVec2{ m_ViewportSize.x, m_ViewportSize.y }, ImVec2{ 0, 0 }, ImVec2{ 1, 1 });
