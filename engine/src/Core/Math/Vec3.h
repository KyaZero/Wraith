...
namespace Wraith
{
    ...
    public:
        ...
        inline T Length() const { return (T)sqrt((T)LengthSqr()); }
        ...
        void Serialize(dubu::serialize::ReadBuffer& buffer) { buffer >> x >> y >> z; }
        void Serialize(dubu::serialize::WriteBuffer& buffer) const { buffer << x << y << z; }
...
{
    template <typename T>
    struct hash<Wraith::Vec3<T>>
    {
        std::size_t operator()(const Wraith::Vec3<T>& vec) const { return std::hash<T>()(vec.x) ^ std::hash<T>()(vec.y) ^ std::hash<T>()(vec.z); }
    };
}  // namespace std