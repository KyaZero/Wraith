...
namespace fw
{
	...
	public:
		...
		static Mat3<T> Inverse(const Mat3<T>& matrix)
		{
			...
			Mat3<T> result{ matrix.m_Numbers[4] * matrix.m_Numbers[8] - matrix.m_Numbers[5] * matrix.m_Numbers[7],
		...
		{
			return Vec3<T>(vec.x * matrix.m_Numbers[0] + vec.y * matrix.m_Numbers[3] + vec.z * matrix.m_Numbers[6],
