...
namespace Wraith
{
    ...
    public:
        ...
        void CreateFromTexture(ID3D11Texture2D* texture) const;
        static void CreateDepth(const Vec2u& size, ImageFormat format = ImageFormat::D32_FLOAT);

        static void Clear(const Vec4f& color = Vec4f());
        static void ClearDepth(f32 depth = 1.0f, u32 stencil = 0);
        ...
        static void SetAsActiveTarget(Texture* depth = nullptr);
        static void SetViewport();
        static void SetCustomViewport(f32 top_left_x, f32 top_left_y, f32 width, f32 height, f32 min_depth = 0.0f, f32 max_depth = 0.0f);

        static void Bind(u32 slot);
        ...
        void Blit(const u8* data, i32 x, i32 y, i32 w, i32 h, i32 stride) const;
